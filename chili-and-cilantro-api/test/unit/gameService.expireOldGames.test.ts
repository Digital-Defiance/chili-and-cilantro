...
describe('GameService', () => {
  ...
  describe('expireGamesOrThrowAsync', () => {
    it('should expire multiple games', async () => {
      ...
      expect(gameService.actionService.expireGameAsync).toHaveBeenCalledTimes(mockGames.length);
    });

    it('should handle errors', async () => {
      // Create a mock game with a failing save method
      const mockGame = generateGame(true, { currentPhase: GamePhase.ROUND_START });
      ...
      await expect(gameService.expireGamesOrThrowAsync([mockGame])).rejects.toThrow('Save failed');
  ...
  describe('performExpireOldGamesAsync', () => {
    it('should find and expire old games', async () => {
      ...
      cutoffDate.setMinutes(cutoffDate.getMinutes() - constants.MAX_GAME_AGE_WITHOUT_ACTIVITY_IN_MINUTES - 1); // Subtracting an extra minute to ensure the date is old enough

      // Create mock old games
      const mockOldGames = [
        generateGame(true, { currentPhase: GamePhase.ROUND_START, lastModified: cutoffDate }),
        generateGame(true, { currentPhase: GamePhase.ROUND_START, lastModified: cutoffDate })
      ];
      ...
      jest.spyOn(gameService, 'expireGamesOrThrowAsync').mockResolvedValue(undefined);
      ...
      expect(gameService.expireGamesOrThrowAsync).toHaveBeenCalledWith(mockOldGames);
    });
    ...
    it('should handle errors during the expiring process', async () => {
      ...
      await expect(gameService.performExpireOldGamesAsync()).rejects.toThrow(mockError);
