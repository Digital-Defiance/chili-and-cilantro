...
class TestTransactionManager extends TransactionManager {
  public async withTransaction<TResult>(work: (session: ClientSession) => Promise<TResult>): Promise<TResult> {
...
describe("transactionManager", () => {
  describe("withTransaction", () => {
    let mockStartSession, mockCommitTransaction, mockAbortTransaction, mockEndSession;
    ...
    beforeEach(() => {
      ...
      mockStartSession = jest.spyOn(mongoose, 'startSession').mockResolvedValue(mockedSession as unknown as ClientSession);
    });
    ...
    it("should rollback the transaction if the callback fails", async () => {
      ...
      await expect(manager.withTransaction(mockCallback)).rejects.toThrow(mockError);
