...
describe('GameService', () => {
  ...
  describe('validateSendMessageOrThrow', () => {
    it('should not throw an error for a valid message length', () => {
      ...
      expect(() => gameService.validateSendMessageOrThrow(message)).not.toThrow();
    });

    it('should throw an error for a message too short', () => {
      const message = 'L'.repeat(constants.MIN_MESSAGE_LENGTH - 1);
      expect(() => gameService.validateSendMessageOrThrow(message)).toThrow(InvalidMessageError);
    });
  ...
  describe('sendMessageAsync', () => {
    it('should successfully send a message', async () => {
      const message = 'L'.repeat(constants.MIN_MESSAGE_LENGTH + 1);
      jest.spyOn(gameService.actionService, 'sendMessageAsync').mockResolvedValue({ /* mock return value */ });
      ...
      expect(gameService.actionService.sendMessageAsync).toHaveBeenCalledWith(game, chef, message);
    });
  });
  ...
  describe('performSendMessageAsync', () => {
    it('should successfully perform sending a message', async () => {
      ...
      jest.spyOn(gameService.chefService, 'getGameChefOrThrowAsync').mockResolvedValue(chef);
      jest.spyOn(gameService, 'validateSendMessageOrThrow').mockImplementation(() => { });
      jest.spyOn(gameService, 'sendMessageAsync').mockResolvedValue({ /* mock return value */ });

      const result = await gameService.performSendMessageAsync(game.code, user, message);
      expect(result).toBeDefined(); // Adjust based on your mock return value
      expect(gameService.validateSendMessageOrThrow).toHaveBeenCalledWith(message);
      expect(gameService.sendMessageAsync).toHaveBeenCalledWith(game, chef, message);
    });
    ...
    it('should throw an error if validation fails', async () => {
      ...
      await expect(gameService.performSendMessageAsync(game.code, user, message))
