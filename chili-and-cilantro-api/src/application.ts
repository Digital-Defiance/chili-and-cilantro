...
export class App implements IApplication {
  ...
  constructor() {
    ...
    this.server = null;
    this.apiRouter = new ApiRouter(this.getModel);
    this.appRouter = new AppRouter(this.apiRouter);
  }
  ...
  public async start(mongoUri?: string, debug = true): Promise<void> {
    ...
    try {
      ...
      this.appRouter.init(this.expressApp, debug);
  ...
  public getModel: GetModelFunction = <T extends IBaseDocument<T>>(
    modelName: string,
  ): Model<T> => {
